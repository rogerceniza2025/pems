        ...
        Object.entries(deps).forEach(([name, version]) => {
          if (typeof version === 'string') {
            ...
            if (versions.has(baseName) && versions.get(baseName) !== version) {
              throw new Error(`Version conflict for ${baseName}: ${versions.get(baseName)} vs ${version}`);
            }
...
describe('Database Configuration', () => {
  ...
  describe('Database Connection', () => {
    ...
    it('should have required extensions installed', async () => {
      const result = await prisma.$queryRaw`SELECT extname FROM pg_extension WHERE extname IN ('uuid-ossp', 'pgcrypto')`;
...
describe('Code Quality Tools', () => {
  ...
  describe('Pre-commit Hooks', () => {
    it('should have husky configuration', () => {
      ...
      expect(readFileSync(path.join(huskyDir, 'pre-commit'), 'utf8')).toBeDefined();
    });
...
test.describe('Database Integration E2E Tests', () => {
  test('should connect to database and perform operations', async ({ request }) => {
    // Test database operations through API
    const response = await request.post('http://localhost:3001/api/test/db-connection', {
      data: { test: true }
    });
  ...
  test('should run migrations successfully', async ({ request }) => {
    const response = await request.post('http://localhost:3001/api/test/migrate', {
      data: { dryRun: true }
    });
...
describe('CI/CD Configuration', () => {
  ...
  describe('GitHub Actions Workflows', () => {
    ...
    it('should have deployment workflow configuration', () => {
      ...
      const workflow = yaml.load(readFileSync(deployWorkflowPath, 'utf8')) as any;
...
test.describe('CI/CD Pipeline E2E Tests', () => {
  test('should execute complete CI workflow', async ({ request }) => {
    ...
    const response = await request.post('https://api.github.com/repos/test/pems/dispatches', {
      data: {
        event_type: 'ci-test',
  ...
  test('should trigger tests on pull request', async ({ page }) => {
    ...
    await page.goto('https://github.com/test/pems/compare/main...feature-branch');
    ...
    await expect(page.locator('[data-testid="ci-status"]')).toContainText('pending');
  });
...
test.describe('Build Artifacts E2E Tests', () => {
  test('should download and verify build artifacts', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="artifact-download"]')).toBeVisible();
  ...
  test('should serve built application correctly', async ({ page }) => {
    ...
    const headers = await page.evaluate(() => {
      return {
        'content-encoding': performance.getEntriesByType('navigation')[0].responseHeaders.find((h: any) => h.name === 'content-encoding')?.value,
        'cache-control': performance.getEntriesByType('navigation')[0].responseHeaders.find((h: any) => h.name === 'cache-control')?.value
      };
    });
