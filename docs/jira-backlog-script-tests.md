...
describe('Configuration Validation', () => {
  describe('validateConfig', () => {
    ...
    it('should throw error when JIRA_URL is missing', () => {
      ...
      expect(() => validateConfig()).toThrow('Missing required environment variables: JIRA_URL');
    });
    ...
    it('should throw error when JIRA_EMAIL is missing', () => {
      ...
      expect(() => validateConfig()).toThrow('Missing required environment variables: JIRA_EMAIL');
    });
    ...
    it('should throw error when JIRA_API_TOKEN is missing', () => {
      ...
      expect(() => validateConfig()).toThrow('Missing required environment variables: JIRA_API_TOKEN');
    });
    ...
    it('should throw error when JIRA_PROJECT_KEY is missing', () => {
      ...
      expect(() => validateConfig()).toThrow('Missing required environment variables: JIRA_PROJECT_KEY');
    });
    ...
    it('should throw error when multiple environment variables are missing', () => {
      ...
      expect(() => validateConfig()).toThrow('Missing required environment variables: JIRA_EMAIL, JIRA_API_TOKEN, JIRA_PROJECT_KEY');
    });
  });
});
```
...
- Integrate with payment gateways`;
    ...
    it('should parse user stories correctly', () => {
      ...
      expect(story.userStory).toBe('As a system administrator, I want to manage multiple schools/tenants, So that each institution has isolated data and configuration.');
    });
    ...
    it('should parse acceptance criteria correctly', () => {
      ...
      expect(story.acceptanceCriteria[0]).toBe('New tenants can be created with unique identifiers');
      expect(story.acceptanceCriteria[1]).toBe('Tenant isolation is enforced at database level');
      expect(story.acceptanceCriteria[2]).toBe('Row-Level Security (RLS) is implemented');
    });
    ...
    it('should parse technical tasks correctly', () => {
      ...
      expect(story.technicalTasks[0]).toBe('Implement tenant management domain module');
...
describe('Jira Client Creation', () => {
  describe('createJiraClient', () => {
    ...
    it('should handle special characters in credentials', () => {
      ...
      const base64Credentials = authHeader.replace('Basic ', '');
      const credentials = Buffer.from(base64Credentials, 'base64').toString();
      expect(credentials).toBe('test+special@example.com:token-with-special-chars-@#$%');
    });
  });
});
```
...
describe('File System Integration', () => {
  describe('readSprintPlan', () => {
    ...
    it('should throw error when file does not exist', () => {
      ...
      expect(() => readSprintPlan('/nonexistent/file.md')).toThrow('ENOENT: no such file or directory');
    });
    ...
    it('should handle file read errors', () => {
      ...
      expect(() => readSprintPlan(testSprintPlanPath)).toThrow('Permission denied');
    });
  });
});
```
...
describe('Jira API Integration', () => {
  describe('createJiraIssue', () => {
    ...
    it('should handle network error', async () => {
      ...
      await expect(createJiraIssue(jiraClient, mockStory, 1)).rejects.toThrow('Network timeout');
    });
...
describe('Complete Workflow Tests', () => {
  describe('populate-jira-backlog.js', () => {
    ...
    it('should complete full workflow in dry-run mode', async () => {
      const result = await execAsync('node scripts/populate-jira-backlog.js --dry-run', {
    ...
    it('should handle specific sprint processing', async () => {
      const result = await execAsync('node scripts/populate-jira-backlog.js --sprint 1 --dry-run', {
        env: { ...process.env, ...testEnv.env }
      });
    ...
    it('should display help correctly', async () => {
      const result = await execAsync('node scripts/populate-jira-backlog.js --help', {
        env: { ...process.env, ...testEnv.env }
      });
    ...
    it('should handle malformed sprint plan', async () => {
      ...
      await fs.writeFile(testEnv.sprintPlanPath, 'Invalid content without proper structure');
