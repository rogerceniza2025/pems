...
export class TenantAwarePrismaClient {
  ...
  clearTenantContext(): void {
    ...
    this.client.$executeRaw`RESET app.current_tenant_id`
    this.client.$executeRaw`RESET app.is_system_admin`
    this.client.$executeRaw`RESET app.current_user_id`
  }
  ...
  private addTenantFiltering(args: any[], method: string): any[] {
    ...
    const skipFiltering = ['count', 'aggregate', 'groupBy', '$queryRaw', '$executeRaw']
    ...
    if (method === 'findMany' || method === 'findFirst' || method === 'findUnique') {
      return [{ where: { tenant_id: this.tenantContext.tenantId } }, ...restArgs]
    }
  ...
  private async configureDatabaseSession(context: TenantContext): Promise<void> {
    try {
      await this.client.$executeRaw`SET app.current_tenant_id = ${context.tenantId}`
      ...
      if (context.userId) {
        await this.client.$executeRaw`SET app.current_user_id = ${context.userId}`
      }
    } catch (error) {
   ...
   */
  async executeTenantAwareQuery<T = any>(query: string, ...params: any[]): Promise<T[]> {
    if (!this.tenantContext) {
      throw new Error('Tenant context must be set before executing tenant-aware queries')
    }
   ...
   */
  async executeTenantAwareCommand(command: string, ...params: any[]): Promise<any> {
    if (!this.tenantContext) {
      throw new Error('Tenant context must be set before executing tenant-aware commands')
    }
  ...
  async transaction<T>(
    ...
    return originalClient.$transaction(async (tx) => {
      ...
      if (this.tenantContext) {
        tenantTx.setTenantContext(this.tenantContext)
      }
 ...
 */
export function createTenantAwareClient(prisma: PrismaClient): TenantAwarePrismaClient {
