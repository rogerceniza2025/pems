 ...
 */
export function expectCSSVariable(element: HTMLElement, property: string, expectedValue: string) {
 ...
 */
export function hasCSSVariable(element: HTMLElement, property: string): boolean {
  const computedStyle = getComputedStyle(element);
 ...
 */
export function expectComputedStyle(element: HTMLElement, property: string, expectedValue: string) {
  const computedStyle = getComputedStyle(element);
  const actualValue = computedStyle.getPropertyValue(property).trim();
  expect(actualValue).toBe(expectedValue);
}
 ...
 */
export function expectResponsiveClasses(element: HTMLElement, _breakpoint: string, classes: string[]) {
 ...
 */
export function expectThemeConsistency(elements: HTMLElement[], property: string) {
  ...
  const firstValue = getComputedStyle(firstElement).getPropertyValue(property).trim();

  elements.forEach((element, index) => {
    if (index > 0) {
      const currentValue = getComputedStyle(element).getPropertyValue(property).trim();
 ...
 */
export function expectColorContrast(foreground: string, background: string, minimumRatio: number = 4.5) {
 ...
 */
export function expectAriaAttribute(element: HTMLElement, attribute: string, value: string) {
...
export const DesignTokenTestUtils = {
   ...
   */
  expectTokenName(token: string, pattern: RegExp = /^--[a-z][a-z0-9]*(-[a-z0-9]+)*$/) {
