...
export function cva<T = Record<string, Record<string, string>>>(
  ...
  return (
    ...
    // Merge base classes
    const baseClasses =
      typeof customBase === 'function' ? customBase(props) : (customBase ?? '')
    const resolvedConfigClassName =
      typeof configClassName === 'function'
        ? configClassName(props)
        : configClassName

    // Generate variant classes
    const variantClasses = Object.entries(variants).map(
      ([variantName, variantOptions]) => {
        const variantValue = (props as Record<string, unknown>)[variantName]
        const defaultValue = (defaultVariants as Record<string, unknown>)?.[
          variantName
        ]
        ...
        if (
          ...
          const options = variantOptions as Record<string, string>
 ...
 */
export type VariantKeys<T extends Record<string, Record<string, string>>> =
  keyof T
...
export function withVariants<T extends Record<string, Record<string, string>>>(
  ...
  return <P extends Record<string, unknown>>(
    ...
    const StyledComponent = (
      props: P & { className?: string; class?: string },
    ) => {
    ...
    StyledComponent.displayName =
      (Component as { displayName?: string }).displayName ?? 'StyledComponent'
 ...
 */
export function mergeCVAConfigs<
  T extends Record<string, Record<string, string>>,
>(baseConfig: CVAConfig<T>, ...configs: Partial<CVAConfig<T>>[]): CVAConfig<T> {
...
export function conditionalVariant<
  T extends Record<string, Record<string, string>>,
>(condition: boolean, variants: Partial<T>) {
...
export function responsiveVariant<
  T extends Record<string, Record<string, string>>,
>(breakpoint: string, variants: T): { [key: string]: T } {
  const screens = {
    xs: '0px',
    sm: '640px',
    md: '768px',
...
export function isValidVariant<
  T extends Record<string, Record<string, string>>,
>(config: CVAConfig<T>, variantName: string, variantValue: string): boolean {
  return (
