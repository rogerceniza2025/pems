...
export const FormError = (props: FormErrorProps) => {
  ...
  return (
    <Show when={hasError()}>
      ...
      >
        {/* Error Icon */}
        <Show when={local.icon !== false}>{local.icon ?? <ErrorIcon />}</Show>
...
export const FormErrorList = (props: {
  ...
  const hasMoreErrors = () => {
    return (
      local.maxItems && local.errors && local.errors.length > local.maxItems
    )
  }
      ...
      >
        ...
        <ul class="text-sm text-destructive space-y-1">
          <For each={errorEntries()}>
            {({ field, message }) => (
              <li>
                {local.showFieldNames && <strong>{field}: </strong>}
                {message}
              </li>
            )}
          </For>
