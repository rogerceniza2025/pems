...
class TTDEnforcer {
  ...
  getSourceFiles() {
    return this.stagedFiles.filter(
      (file) =>
  ...
  validateNewCodeCoverage() {
    ...
    try {
      // Run tests on current changes - fall back to basic test if changed files not supported
      try {
        execSync(`pnpm test:changed`, { encoding: 'utf8', stdio: 'pipe' })
      } catch {
        // Fallback to running basic coverage if --changed isn't working
  ...
  run() {
    ...
    if (!success) {
      logError(
        'TDD validation failed. Please fix the violations before committing.',
      )
      process.exit(1)
    }
