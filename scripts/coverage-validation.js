...
try {
  const { CoverageCollector } = require('./analytics/collectors/CoverageCollector.js');
...
class CoverageValidator {
  ...
  async getCoverageReport() {
    try {
      ...
      const lines = execSync('pnpm test:coverage -- --reporter=json', { encoding: 'utf8' }).split('\n');
  ...
  async collectAnalyticsData() {
    try {
      ...
      const collector = new analyticsCollector.CoverageCollector(storage, analyticsConfig);
