...
function parseSprintPlan(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentStory && line.startsWith('**As a**')) {
      ...
      const userStoryLines = [line.replace(/\*\*/g, '')]
...
function formatDescription(story) {
...
async function main() {
  console.log(
    `${COLORS.blue}🚀 Jira Backlog Population Script${COLORS.reset}\n`,
  )

  if (options.dryRun) {
    console.log(
      `${COLORS.yellow}🔍 DRY RUN MODE - No issues will be created${COLORS.reset}\n`,
    )
  } else {
    validateConfig()
  }
  ...
  try {
    ...
    if (filteredSprints.length === 0) {
      console.log(
        `${COLORS.yellow}No sprints found matching the criteria.${COLORS.reset}`,
      )
      return
    }

    // Count total stories
    const totalStories = filteredSprints.reduce(
      (sum, sprint) => sum + sprint.stories.length,
      0,
    )
    console.log(
      `${COLORS.cyan}Found ${filteredSprints.length} sprint(s) with ${totalStories} stories total${COLORS.reset}\n`,
    )
    ...
    if (!options.dryRun) {
      ...
      console.log(
        `${COLORS.blue}Connected to Jira at ${CONFIG.JIRA_URL}${COLORS.reset}\n`,
      )
    }
    ...
    for (const sprint of filteredSprints) {
      console.log(
        `${COLORS.cyan}Sprint ${sprint.number}: ${sprint.title}${COLORS.reset}`,
      )
      ...
      for (const story of sprint.stories) {
        try {
          if (options.dryRun) {
            console.log(
              `  ${COLORS.yellow}→${COLORS.reset} ${story.title} (sprint-${sprint.number})`,
            )
            successCount++
          } else {
            console.log(
              `  ${COLORS.blue}Creating:${COLORS.reset} ${story.title}...`,
            )
            ...
            console.log(
              `  ${COLORS.green}✅ Created issue ${result.key}: ${story.title} (sprint-${sprint.number})${COLORS.reset}`,
            )
          ...
          console.error(
            `  ${COLORS.red}❌ Failed to create issue "${story.title}": ${error.message}${COLORS.reset}`,
          )
          failCount++
        }
      }
    ...
    if (options.dryRun) {
      ...
      console.log(
        `  Successfully created: ${COLORS.green}${successCount}${COLORS.reset}`,
      )
